...
def delete_image_from_directory():
    """
    deleting old image every time after returning to home page
    """
    filename_delete = "image.jpg"
    image_path = app.config['UPLOAD_FOLDER'] + filename_delete
...
def allowed_file(file):
    ...
    """
    if file[-3:].lower() in ALLOWED_EXTENSIONS:
        return True
    else:
        return False
...
def load_picture():
    ...
    try:
        load_file = "image.jpg"
        img = cv2.imread(os.path.join(app.config['UPLOAD_FOLDER'], load_file), -1)
...
def kernels(option):
    ...
    if option in all_kernels:
        return all_kernels[option]
    else:
        return None
        ...
        try:
            if request.files and request.form:
                ...
                if allowed_file(str(image.filename)) and upload:
                    ...
                    curr_filename = "image.jpg"
                    cv2.imwrite((os.path.join(app.config['UPLOAD_FOLDER'], curr_filename)), img)
                    return '', 204
                else:
...
def image_processing():
    ...
    applying a selected kernel to the picture saving and displaying using display_image() function
    """
    try:
        if request.method == 'POST':
            img = load_picture()
            ...
            if kernel.size:
                ...
                curr_filename = "image.jpg"
                plt.imsave((os.path.join(app.config['UPLOAD_FOLDER'], curr_filename)), img, cmap=cmap)
                return render_template('processed_image.html', filename=curr_filename, kernel=option)
        else:
            abort(404)
...
def display_image(filename):
    ...
    try:
        return send_from_directory(
            ...
            directory=app.config['PROCESSED_IMAGE'], path=filename, as_attachment=True
        )
